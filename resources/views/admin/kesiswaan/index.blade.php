    ...
    <div class="section-body">
      ...
      <div class="card">
        <div class="card-header">
          <h4><i class="fas fa-book-open"></i> Kesiswaan </h4>
          <div class="card-header-action">
            @if ($cek_kesiswaan !== 1)
            <a href="{{ route('admin.kesiswaan.create') }}" class="btn btn-success">
        ...
        <div class="card-body">
          <div class="table-responsive">
            <table class="table table-bordered">
              ...
              <tbody>
              @forelse ($kesiswaans as $no => $kesiswaan)
                <tr>
                  <th style="text-align:center">
                    {{ $loop->iteration }}
                  </th>

                  <td>{{ $kesiswaan->name }}</td>
                  <td>{!! $kesiswaan->content !!}</td>
                  <td>
                    @if($kesiswaan->image)
                  ...
                  <td class="text-center">
                      ...
                      <button type="button" class="btn btn-sm btn-danger"
                              onclick="confirmDelete({{ $kesiswaan->id }})">
                        <i class="fa fa-trash"></i>
                      </button>
                      {{-- form delete akan disubmit setelah konfirmasi --}}
                      <form id="delete-form-{{ $kesiswaan->id }}"
                            action="{{ url('/admin/kesiswaan/'.$kesiswaan->id) }}"
            ...
            <div style="text-align:center">
              {{ $kesiswaans->links('vendor.pagination.bootstrap-5') }}
            </div>
          </div>
        </div>
      </div>
...
<script>
  function confirmDelete(id) {
    ...
    const url   = form ? form.getAttribute('action') : ('/admin/kesiswaan/' + id);
